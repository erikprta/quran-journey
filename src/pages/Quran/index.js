...
const AyahCard = ({ number, name, translation, revelation, numberOfAyahs }) => {
  return (
    <Link
      to={`/quran/${number}`}
      className="border rounded-md p-4 group hover:border-green-500 hover:bg-slate-50 transition-colors flex gap-4 items-center w-full ">
      <span className="h-10 max-w-[2.5rem] w-full text-sm bg-slate-200 transition-colors group-hover:bg-green-400 group-hover:text-white flex items-center justify-center font-semibold rounded-full">
        {number}
      </span>
